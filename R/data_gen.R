...
##' @param herd_samp_frac The total sampling fraction at the herd level
##' @param herd_samp_dist The fraction of samples to be collected from each herd risk group
...
sample_data <- function(nherds = 500,
    ...
    herd_urg <- sample(x = factor(seq_len(n_herd_urg)), size = nherds, replace = TRUE, prob = herd_dist)
    herds <- data.frame(ppn, N, n, herd_urg, stringsAsFactors = FALSE)
    ...
    herds <- do.call("rbind", lapply(ppn, function(x){
        ...
        if(any(n_sample_animal_urg > N_animal_urg)) {
            n_sample_animal_urg[n_sample_animal_urg > N_animal_urg] <- N_animal_urg[n_sample_animal_urg > N_animal_urg] 
        }
